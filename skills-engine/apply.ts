...
import { AGENTOS_DIR, STATE_FILE } from './constants.js';
import { copyDir } from './fs-utils.js';
import { isCustomizeActive } from './customize.js';
import { initAgentOSDir } from './init.js';
...
export async function applySkill(skillDir: string): Promise<ApplyResult> {
  ...
  const statePath = path.join(projectRoot, AGENTOS_DIR, STATE_FILE);
  ...
  for (const relPath of manifest.modifies) {
    ...
    const basePath = path.join(projectRoot, AGENTOS_DIR, 'base', resolvedPath);
  ...
  try {
    ...
    for (const relPath of manifest.modifies) {
      ...
      const tmpCurrent = path.join(
        os.tmpdir(),
        `agentos-merge-${crypto.randomUUID()}-${path.basename(relPath)}`,
      );
