...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'agentos-test-'));
}

export function setupAgentOSDir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.agentos', 'base', 'src'), {
    recursive: true,
  });
  fs.mkdirSync(path.join(tmpDir, '.agentos', 'backup'), { recursive: true });
}

export function writeState(tmpDir: string, state: any): void {
  const statePath = path.join(tmpDir, '.agentos', 'state.yaml');
