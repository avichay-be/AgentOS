...
describe('rebase', () => {
  ...
  it('rebase with one skill: patch created, state updated, rebased_at set', async () => {
    // Set up base file
    const baseDir = path.join(tmpDir, '.agentos', 'base', 'src');
    ...
    const patchPath = path.join(tmpDir, '.agentos', 'combined.patch');
    ...
    const stateContent = fs.readFileSync(
      path.join(tmpDir, '.agentos', 'state.yaml'),
      'utf-8',
    );
  ...
  it('rebase flattens: base updated to match working tree', async () => {
    ...
    const baseContent = fs.readFileSync(
      path.join(tmpDir, '.agentos', 'base', 'src', 'index.ts'),
      'utf-8',
    );
  ...
  it('rebase with multiple skills + custom mods: all collapsed into single patch', async () => {
    // Set up base files
    const baseDir = path.join(tmpDir, '.agentos', 'base');
      ...
      custom_modifications: [
        {
          ...
          patch_file: '.agentos/custom/001-tweaked-config.patch',
        },
      ],
    });
    ...
    const patchContent = fs.readFileSync(
      path.join(tmpDir, '.agentos', 'combined.patch'),
      'utf-8',
    );
    ...
    const baseConfig = fs.readFileSync(
      path.join(tmpDir, '.agentos', 'base', 'src', 'config.ts'),
      'utf-8',
    );
