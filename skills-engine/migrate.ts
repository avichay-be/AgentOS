...
import { BASE_DIR, CUSTOM_DIR, AGENTOS_DIR } from './constants.js';
import { initAgentOSDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initAgentOSDir();
  console.log('Skills system initialized. .agentos/ directory created.');
}
...
export function migrateExisting(): void {
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      console.log(
        'Custom modifications captured in .agentos/custom/migration.patch',
      );
    } else {
