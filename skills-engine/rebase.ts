...
import { BASE_DIR, AGENTOS_DIR } from './constants.js';
...
export async function rebase(newBasePath?: string): Promise<RebaseResult> {
  ...
  try {
    ...
    const stateFilePath = path.join(projectRoot, AGENTOS_DIR, 'state.yaml');
    ...
    try {
      ...
      // Save combined patch
      const patchPath = path.join(projectRoot, AGENTOS_DIR, 'combined.patch');
      ...
      if (newBasePath) {
        ...
        for (const relPath of trackedFiles) {
          ...
          const tmpSaved = path.join(
            os.tmpdir(),
            `agentos-rebase-${crypto.randomUUID()}-${path.basename(relPath)}`,
          );
