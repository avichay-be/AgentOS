...
│  │    • mcp__agentos__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── agentos.log               # Host stdout
│   └── agentos.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.agentos.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'agentos-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your agentos installation
...
AgentOS uses a hierarchical memory system based on CLAUDE.md files.
...
AgentOS has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__agentos__schedule_task]
        {
...
### AgentOS MCP (built-in)

The `agentos` MCP server is created dynamically per agent call with the current group's context.
...
AgentOS runs as a single macOS launchd service.

### Startup Sequence

When AgentOS starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned AgentOS containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/agentos.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/agentos.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.agentos.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.agentos.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.agentos.plist
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep agentos` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; AgentOS auto-starts container runtime but may fail |
...
- `logs/agentos.log` - stdout
- `logs/agentos.error.log` - stderr
