# Slack App Setup for AgentOS

Step-by-step guide to creating and configuring a Slack app for use with AgentOS.

## Prerequisites

- A Slack workspace where you have admin permissions (or permission to install apps)
- Your AgentOS instance with the `/add-slack` skill applied
...
3. When prompted for a token name, enter something like `agentos`
4. Click **Generate**
...
You need the Slack channel ID to register it with AgentOS.
...
The AgentOS JID format is `slack:` followed by the channel ID, e.g., `slack:C0123456789`.
