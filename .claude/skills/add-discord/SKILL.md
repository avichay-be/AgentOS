...
This skill adds Discord support to AgentOS using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.agentos/state.yaml`. If `discord` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
- @mention translation (Discord `<@botId>` → AgentOS trigger format)
