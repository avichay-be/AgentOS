...
Read `.agentos/state.yaml`. If `ollama` is in `applied_skills`, skip to Phase 3 (Configure). The code changes are already in place.
...
```bash
tail -f logs/agentos.log | grep -i ollama
```
