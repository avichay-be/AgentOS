#!/bin/bash
# Watch AgentOS IPC for Ollama activity and show macOS notifications
...
while true; do
  ...
  for status_file in data/ipc/*/ollama_status.json; do
    ...
    case "$STATUS" in
      generating)
        osascript -e "display notification \"$DETAIL\" with title \"AgentOS → Ollama\" sound name \"Submarine\"" 2>/dev/null
        echo "$(date +%H:%M:%S) 🔄 $DETAIL"
        ;;
      done)
        osascript -e "display notification \"$DETAIL\" with title \"AgentOS ← Ollama ✓\" sound name \"Glass\"" 2>/dev/null
        echo "$(date +%H:%M:%S) ✅ $DETAIL"
        ;;
      listing)
