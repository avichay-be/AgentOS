...
- Added: `gmail` MCP server alongside the existing `agentos` server:
  ```
  gmail: {
    command: 'npx',
...
## Invariants
- The `agentos` MCP server configuration is unchanged
...
## Must-keep
- The `agentos` MCP server with its environment variables
