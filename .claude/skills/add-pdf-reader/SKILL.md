---
name: add-pdf-reader
description: Add PDF reading to AgentOS agents. Extracts text from PDFs via pdftotext CLI. Handles WhatsApp attachments, URLs, and local files.
---
...
Read `.agentos/state.yaml`. If `add-pdf-reader` is in `applied_skills`, skip to Phase 3 (Verify).

## Phase 2: Apply Code Changes

### Initialize skills system (if needed)

If `.agentos/` directory doesn't exist:
...
- Records application in `.agentos/state.yaml`
...
```bash
tail -f logs/agentos.log | grep -i pdf
```
