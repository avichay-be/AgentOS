...
Adds Parallel AI MCP integration to AgentOS for advanced web research capabilities.
...
- **Non-blocking Design** - Uses AgentOS scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. AgentOS already set up and running
...
Add Parallel AI MCP servers after the agentos server:
```typescript
const mcpServers: Record<string, any> = {
  agentos: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__agentos__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__agentos__send_message
   4. Use mcp__agentos__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo agentos-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep agentos  # macOS
...
- Check task runs: `tail -f logs/agentos.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.agentos` (macOS) or `systemctl --user restart agentos` (Linux)
