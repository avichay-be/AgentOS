...
- Linux: `systemctl --user stop agentos` (or `systemctl stop agentos` if root)

Run `npx tsx setup/index.ts --step service` and parse the status block.

**If FALLBACK=wsl_no_systemd:** WSL without systemd detected. Tell user they can either enable systemd in WSL (`echo -e "[boot]\nsystemd=true" | sudo tee /etc/wsl.conf` then restart WSL) or use the generated `start-agentos.sh` wrapper.
...
- macOS: check `launchctl list | grep agentos`. If PID=`-` and status non-zero, read `logs/agentos.error.log`.
...
- SERVICE=stopped → `npm run build`, then restart: `launchctl kickstart -k gui/$(id -u)/com.agentos` (macOS) or `systemctl --user restart agentos` (Linux) or `bash start-agentos.sh` (WSL nohup)
...
Tell user to test: send a message in their registered chat. Show: `tail -f logs/agentos.log`

## Troubleshooting

**Service not starting:** Check `logs/agentos.error.log`. Common: wrong Node path (re-run step 7), missing `.env` (step 4), missing channel credentials (re-invoke channel skill).

**Container agent fails ("Claude Code process exited with code 1"):** Ensure the container runtime is running — `open -a Docker` (macOS Docker), `container system start` (Apple Container), or `sudo systemctl start docker` (Linux). Check container logs in `groups/main/logs/container-*.log`.

**No response to messages:** Check trigger pattern. Main channel doesn't need prefix. Check DB: `npx tsx setup/index.ts --step verify`. Check `logs/agentos.log`.

**Channel not connecting:** Verify the channel's credentials are set in `.env`. Channels auto-enable when their credentials are present. For WhatsApp: check `store/auth/creds.json` exists. For token-based channels: check token values in `.env`. Restart the service after any `.env` change.

**Unload service:** macOS: `launchctl unload ~/Library/LaunchAgents/com.agentos.plist` | Linux: `systemctl --user stop agentos`
