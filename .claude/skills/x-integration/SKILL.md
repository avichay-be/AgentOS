---
name: x-integration
description: X (Twitter) integration for AgentOS. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** AgentOS v1.0.0. Directory structure may change in future versions.
...
1. **AgentOS is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep agentos (macOS) or systemctl --user status agentos (Linux)
```
...
| `AGENTOS_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/agentos.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into AgentOS, make the following modifications:
...
All paths below are relative to project root (`AGENTOS_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep agentos  # macOS — should show PID and exit code 0 or -
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/agentos.log | tail -20

# Script errors
grep -i "error\|failed" logs/agentos.log | tail -20
```
...
docker run agentos-agent ls -la /app/src/skills/
```
