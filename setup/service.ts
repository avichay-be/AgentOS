...
<dict>
    ...
    <string>${projectRoot}/logs/agentos.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/agentos.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.agentos');
  } catch {
...
/**
 * Kill any orphaned agentos node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned agentos processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/agentos.service';
    ...
    unitPath = path.join(unitDir, 'agentos.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=AgentOS Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/agentos.log
StandardError=append:${projectRoot}/logs/agentos.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned agentos processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable agentos`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start agentos`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active agentos`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-agentos.sh');
  const pidFile = path.join(projectRoot, 'agentos.pid');

  const lines = [
    '#!/bin/bash',
    '# start-agentos.sh — Start AgentOS without systemd',
    ...
    '    echo "Stopping existing AgentOS (PID $OLD_PID)..."',
    ...
    `  >> ${JSON.stringify(projectRoot + '/logs/agentos.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/agentos.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "AgentOS started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/agentos.log"`,
  ];
